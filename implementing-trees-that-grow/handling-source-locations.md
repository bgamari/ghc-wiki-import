...
- When constructing/generating terms the first field of the constructors should have explicitly mention the source location
  (see the `par` function in the Solution A's code, where the first field of `Par` should have a `SrcSpan`, eventhough a dummy one.)
