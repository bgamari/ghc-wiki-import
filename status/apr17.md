...
\subsubsection{Major changes in GHC 8.2}

While the emphasis of 8.2 is on performance, stability, and consolidation, it
also includes a number of new features.

\paragraph{Libraries, source language, and type system}

\begin{itemize}
  \item \textbf{Indexed Typeable representations.} While GHC has long supported
    runtime type reflection through the \texttt{Typeable} typeclass, its current
    incarnation requires careful use, providing little in the way of type-safety.
    For this reason the implementation of types like \texttt{Data.Dynamic} must be
    implemented in terms of \texttt{unsafeCoerce} with no compiler verification.
    ...
    This allows facilities like \texttt{Data.Dynamic} to be implemented in a fully type-safe
    manner. See the paper\footnote{\url{https://research.microsoft.com/en-us/um/people/simonpj/papers/haskell-dynamic/}}
    for a description of the proposed interface and the Wiki\footnote{\url{https://ghc.haskell.org/trac/ghc/wiki/Typeable/BenGamari}} for
    current implementation status.

  \item \textbf{Backpack.} Backpack has merged with GHC, Cabal and
\texttt{cabal-install}, allowing you to write libraries which are parametrized
...
new major mode \texttt{ghc --backpack}; at the Cabal syntax level, there are two
new fields \texttt{signatures} and \texttt{mixins} which permit you to define parametrized
packages, and instantiate them in a flexible way. More details are on the
Backpack wiki page\footnote{\url{https://ghc.haskell.org/trac/wiki/Backpack}}.

  \item \textbf{Levity polymorphism.} GHC 8.0 reworked GHC's kind system
    ...
    \footnote{\url{https://www.microsoft.com/en-us/research/publication/levity-polymorphism/}}.

  \item \textbf{\texttt{deriving} strategies.} GHC now provides the programmer with a
precise mechanism to distinguish between the three ways to derive typeclass
instances: the usual way, the \texttt{GeneralizedNewtypeDeriving} way, and the
\texttt{DeriveAnyClass} way. See the \texttt{DerivingStrategies} Wiki page for
more details
\footnote{\url{https://ghc.haskell.org/trac/wiki/DerivingStrategies}}.

  \item \textbf{New classes in \texttt{base}.} The \texttt{Bifoldable}, and
\texttt{Bitraversable} typeclasses are now included in the \texttt{base}
library.

  \item \textbf{Unboxed sums.} GHC 8.2 has a new language extension,
\texttt{UnboxedSums}, that enables unboxed representation for non-recursive sum
...
the Wiki page
\footnote{\url{https://ghc.haskell.org/trac/ghc/wiki/UnpackedSumTypes}}.
\end{itemize}

\paragraph{Runtime system}

\begin{itemize}
  \item \textbf{Compact regions}. This runtime system feature allows a
...
\footnote{\url{http://ezyang.com/papers/ezyang15-cnf.pdf}} for details.

  \item \textbf{Better profiling support.} The cost-center profiler now better
...
program events. Moreover, the cost center stack output (e.g. \texttt{.prof}
files) can now be produced in a machine-readable JSON format for easier
integration with external tooling.

  \item \textbf{More robust DWARF output.} GHC 8.2 will be the first release
...
the GHC Wiki\footnote{\url{https://ghc.haskell.org/trac/ghc/wiki/DWARF/Status}}
for further information.

  \item \textbf{Better support for NUMA platforms.} Machines with non-uniform
...
systems.
  ...
  \item \textbf{Experimental changes to the scheduler} that enable the number of
threads used for garbage collection to be lower than the \texttt{-N} setting.

  \item \textbf{Support for \texttt{StaticPointers} in GHCi.} At long last
programs making use of the \texttt{StaticPointers} language extension will have
first-class bytecode interpreter support, allowing such programs to be loaded
into GHCi.

  \item \textbf{Reduced CPU usage at idle.} A long-standing regression resulting
...
\begin{itemize}
  \item \textbf{Compiler Determinism.} GHC 8.0.2 is the first release of GHC
...
\footnote{\url{https://ghc.haskell.org/trac/ghc/wiki/DeterministicBuilds}} for
details.
\end{itemize}

\subsubsection{Development updates and acknowledgments}
...
remoting support to the \texttt{-fexternal-interpreter} feature introduced in GHC
...
\texttt{COMPLETE} pragma mentioned above.
...
\texttt{nofib} benchmark suite. This suite is one of the primary windows into
...
and generally making \texttt{nofib} a nicer place. In addition, he has been
gradually cleaning up native code generator, revisiting GHC's use of the
\texttt{hoopl} library. Also working in the native code generator is Thomas
...
\footnote{\url{https://www.microsoft.com/en-us/research/publication/compiling-without-continuations}}.
...
process\footnote{\url{https://github.com/ghc-proposals/ghc-proposals}}
officially began in December and has been extremely active ever since: the
\texttt{ghc-proposals} repository now
...
between, please come speak to us either on IRC (\texttt{\#ghc} on
\texttt{irc.freeenode.net}) or \texttt{ghc-devs@haskell.org}. Happy Haskelling!
...
\begin{itemize}
  ...
  \item GHC users guide:

     \url{https://downloads.haskell.org/~ghc/master/users_guide/}
  \item \texttt{ghc-devs} mailing list:

     \url{https://mail.haskell.org/mailman/listinfo/ghc-devs}
\end{itemize}
```