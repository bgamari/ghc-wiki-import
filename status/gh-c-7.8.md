...
- Jan Stolarek has completed the [boolean-primop story](prim-bool).  But we need a shim package. See [
...
- The Applicative-Monad warnings would preferably be integrated, see [\#8004](https://gitlab.staging.haskell.org/ghc/ghc/issues/8004). David Luposchainsky is driving this. (Current status: Warnings implemented, GHC compiles AMP-warning-free. What's left to do is getting the testsuite up to date with this. 13th is a very generous estimate.) **Austin is doing this as we speak**.
