...
>
>
> Question - if a capability can consist of multiple OS threads then in THREADED\_RTS, where is the list of current threads in execution? 
>
>
...
A TSO is treated like any othe object by the GC and it resides in the heap. A TSO is always part of its mu list. A TSO is considered clean if it does not contain pointers to previous generations. This happens in the case where a TSO went through a GC and the objects it refers to got promoted. It then didnt allocate any memory since (maybe it didnt get scheduled) till the next GC. In that case it is considered "clean" and its dirty bit (tso-\>flags & TSO\_DIRTY) is not set. 
