...
- Add `-ticky` to the command line when linking, so that you link against a version of the runtime system that allows you to display the results.  In fact, in the link phase `-ticky` implies `-debug` (see [\#3439](https://gitlab.staging.haskell.org/ghc/ghc/issues/3439)), so you get the debug version of the runtime system too.
...
- tickets: [\#607](https://gitlab.staging.haskell.org/ghc/ghc/issues/607) [\#2455](https://gitlab.staging.haskell.org/ghc/ghc/issues/2455) [\#3439](https://gitlab.staging.haskell.org/ghc/ghc/issues/3439)
