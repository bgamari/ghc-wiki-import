...
* The compiler passes work like this
  ...
  * '''Renaming''': in `(static e)`, check that all free variables of `e` are bound at top level[[BR]][[BR]]
  * '''Typechecking'''. In the type checker, we add `Typeable a` to the set of constraints.[[BR]][[BR]]
