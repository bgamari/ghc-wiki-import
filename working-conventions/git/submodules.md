...
So, in order to make change to a submodule you can either:
 ...
 2) Checkout the respective branch the commit is supposed to be pointed at from (normally `master`. See the table on [wiki:WorkingConventions/Git/Submodules the submodules page] for the full branch/repo summary). 
...
[url "ssh://<user>@github.com/ghc/packages-"]
	insteadOf = ssh://<user>@github.com/ghc/packages/
[url "<user>@github.com:/ghc/packages-"]
	insteadOf = <user>@github.com:/ghc/packages/
}}}
