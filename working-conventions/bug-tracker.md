 ...
 * '''Test Case''': fill in this field with the name of the test in the test suite.  Typically every bug
   closed should have an appropriate test case added to the test suite.[[BR]][[BR]]
 ...
 * All `merge` tickets: https://ghc.haskell.org/trac/ghc/query?status=merge&order=priority
 * All `patch` tickets (also available on the sidebar): https://ghc.haskell.org/trac/ghc/query?status=patch&order=priority
 * All `upstream` tickets: https://ghc.haskell.org/trac/ghc/query?status=upstream&order=priority
```
