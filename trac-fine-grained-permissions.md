...
[
authz\_policy.py](http://trac.edgewall.org/intertrac/source%3Abranches/0.12-stable/tracopt/perm/authz_policy.py) for details. 
...
See also [
sample-plugins/permissions](http://trac.edgewall.org/intertrac/source%3Abranches/0.12-stable/sample-plugins/permissions) for more examples.
...
- Install [
  ConfigObj](http://www.voidspace.org.uk/python/configobj.html) (still needed for 0.12).
- Copy authz\_policy.py into your plugins directory (only for Trac 0.11).
- Put a [
...
\#5648](http://trac.edgewall.org/intertrac/ticket%3A5648)). Patches are partially available, see forgotten authz\_policy.2.patch  part of [
...
That kind of fine-grained permission control needs a definition file, which is the one used by Subversion's mod\_authz\_svn. 
