...
Informally the `-XStrictData` language extension switches data type declarations to be strict by default (what is currently possible using `BangPatters`) allowing fields to be lazy by adding a `~` in front of the field.
