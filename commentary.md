...
Also check out the [wiki:ReadingList GHC Reading List], which gives lots of background reading that will help you understand the actual implementation.
 ...
 * Notes on proposed or in progress (but out of tree) GHC compiler features:
   * [wiki:StrictLanguage Making Haskell strict]
   ...
   * [wiki:Roles Roles]: Fixing the `-XGeneralizedNewtypeDeriving` bug.
   * [wiki:Typeable]: Making Typeable more expressive
   * [wiki:OneShot] An explicit magic `oneShot` function.
