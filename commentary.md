 ...
 * Notes on implemented GHC features:
   ...
   * Vector instructions: [wiki:SimdLlvm Adding SIMD instructions], [wiki:SIMDPlan Implementation plan], [wiki:VectorComputing]
   ...
   * [wiki:KindFact A kind for facts (e.g. class constraints). Implemented as ConstraintKinds]
   * [wiki:GhcKinds Kind level]: Notes on `-XPolyKinds`

 * Notes on proposed or in progress (but out of tree) GHC compiler features:
   * [wiki:Records Simple records]
   ...
   * [wiki:DeferErrorsToRuntime Deferring compilation type errors to runtime]
