...
```trac
This is a proposal (formerly named ''pattern families'') for extending pattern synonyms (PatternSynonyms) allowing patterns to depend on expressions. The implementation is a straightforward desugaring into pattern synonyms and view patterns (ViewPatterns) so familiarity with those two extensions is recommended before reading the proposal.
