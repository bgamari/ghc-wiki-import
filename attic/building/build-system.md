 ...
 * The second section defines the standard
   ...
   [[ref(the main mk/target.mk file)]].
 ...
 * The last section includes a second file of standard
   ...
   to make the [wiki:Building/Using#StandardTargets standard targets].  Why, you ask, can't this
   standard code be part of
   {{{boilerplate.mk}}}?  Good question.  We
   discuss the reason later, in [[ref(Boilerplate architecture)]].
   [[br]][[br]]
   You do not ''have'' to
   {{{include}}} the
 ...
 * {{{make}}} looks in the current directory
   ...
   {{{EXCLUDED_SRCS}}} in [[ref(the mk/boilerplate.mk file)]]).
...
defines standard pattern rules--see [[ref(Pattern rules and options)]].
