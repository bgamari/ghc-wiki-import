...
`HS_SRCS`, now we have one of these for each directory in the source tree,
 ...
 * '''Phase 1''': invoke `inc.mk` again (without `PHASE=0`).  Now we can be sure 
   ...
   repeating Phase 1.
We could have abandoned '''make''''s automatic re-invocation mechanism altogether,
and used three phases, but in practice it's very convenient to use the automatic
