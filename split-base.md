...
- (P2) Prelude goes to a separate shim package, base-prelude. Packages that want to only use base-io have no Prelude to use (see (P1)). Allows packages to mix the shim-packages easily with other, non-standard Prelude providing packages, e.g. classy-prelude.
...
- (I1) It automatically imports \_all\_ visible Prelude modules. So base-pure provides the pure Prelude and base-io adds the IO functions.
