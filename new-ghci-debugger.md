...
- when showing a SrcSpan, we could say which top-level function contains it.  Ideally we should store the declPath that
  the coverage pass collects, but as a \[OccName\] rather than \[String\].  :history, :show context etc. should show the
  enclosing top-level function (or declpath).
