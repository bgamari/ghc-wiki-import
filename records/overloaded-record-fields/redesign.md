 ...
 * GHC will generate a `HasField` instance whenever it currently generates a selector.  Not every field has a selector today, because of existentials (see [https://downloads.haskell.org/~ghc/latest/docs/html/users_guide/data-type-extensions.html#existential-records user manual]).  Moreover, it will only generate a `HasField` instance if the field type is of rank 1, with all the foralls at the top.  (Otherwise we would need impredicative polymorphism.)
