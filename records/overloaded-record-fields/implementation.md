...
The `Has` and `Upd` classes, and `GetResult` and `SetResult` type families, are defined in the module [
...
- Minimise contents of `AvailInfo` (preferably just a list of field `OccName`s?): use `newGlobalBinder` or `lookupOrig` to lookup cached mangled names for selectors and pieces
