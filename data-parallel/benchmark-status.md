...
[[br]]
 ...
 [[br]]
 [http://darcs.haskell.org/packages/dph/examples/smvm/ SMVM] '''(BROKEN - Fusion)'''::
...
[[br]]
 [http://darcs.haskell.org/libraries/dph/dph-examples/spectral/QuickSort/ QuickSort] '''(BROKEN - SpecConstr) (SLOWDOWN)'''::
...
These programs also use user defined algebraic data types. Vectorization of these programs is still a work in progress.
 ...
 [http://darcs.haskell.org/libraries/dph/dph-examples/imaginary/Words/ Words] '''(BROKEN - SpecConstr)'''::
 ...
 '''Status''': Sequential vectorised version does not compile due to blowup in SpecConstr.[[br]]
 '''Todo''': Generate some larger test data. Right now it's just got a small test string baked into the program.

[[br]]
 [http://darcs.haskell.org/libraries/dph/dph-examples/real/NBody/ BarnesHut] '''(BROKEN -fllvm)''' '''(SLOWLORIS)'''::
  ...
  '''Status''':  -fasm vesions compile but fusion doesn't work so it's very slow. LLVM versions take 30min to compile. [[br]]
