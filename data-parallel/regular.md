...
even elements in the array (1) using `fold`. The function `fold` is mappable, and returns an array. If the argument is a one-dimensional
...
{{{
  selectEvenElems (DArray  (sh :*: n) f) = DArray (sh :*: newSize) <......>
     where
          newSize = toScalar $ fold (+) 0 $ map (\x -> if (even x) then 1 else 0) arr  -- (1)
}}}
