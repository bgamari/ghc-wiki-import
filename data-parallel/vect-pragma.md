...
This pragma can also be used for imported functions `f`.  In this case, `f_v` and a suitable vectorisation mapping of `f` to `f_v` is exported iff `f` is re-exported; otherwise, it is only available in the module in which the pragma is located.  By re-exporting a vectorised imported function, we can vectorise functions of modules that have not been compiled with `-fvectorise`.  This is crucial to using the standard `Prelude` in vectorised code.
