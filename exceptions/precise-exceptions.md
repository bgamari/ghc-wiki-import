...
So, looking at [\#11555](https://gitlab.staging.haskell.org/ghc/ghc/issues/11555), `catchIO# undefined` throws an exception that is not caught;
...
reasons discussed in [\#11555](https://gitlab.staging.haskell.org/ghc/ghc/issues/11555), `catch#` should be lazy so that `catch# (f x)` does not
...
the handler an indication (`0#` or `1#`) of whether the exception was precise or imprecise.
