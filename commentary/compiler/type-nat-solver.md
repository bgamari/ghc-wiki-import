...
proof may depend on proofs of as yet unsolved "wanted" constraints.
GHC does not associate any proof terms with "derived" constraints.
...
```wiki
    ...
    1. Add [W] C: 5 + 3 ~ x to inert set
    2. Generate new derived:
     [D] Add_fun(C,Add_def) : x ~ 8   (proof discarded)
    3. GHC uses this hint to improve the wanted to [W] C: 5 + 3 ~ 8
    4. Solved:
     [W] C = Add_def(5,3)
```