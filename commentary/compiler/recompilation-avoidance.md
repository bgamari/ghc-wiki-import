 ...
 * The object file and [wiki:Commentary/Compiler/IfaceFiles interface file] exist from a previous compilation
 * Recompilation is sure to produce exactly the same results, so it
   is not necessary.

If both of these hold, GHC stops compilation early.
...
aim to reduce the amount of unnecessary recompilation that happens.
