...
During type checking, GHC maintains a ''type environment'' whose type definitions are fixed in the module [[GhcModule(compiler/typecheck/TcRnTypes.lhs)]] with the operations defined in [[GhcModule(compiler/typecheck/TcEnv.lhs)]]. Among other things, the environment contains all imported and local instances as well as a list of ''global'' entities (imported and local types and classes together with imported identifiers) and ''local'' entities (locally defined identifiers). This environment is threaded through the [wiki:Commentary/Compiler/TcRnMonad type checking monad].
