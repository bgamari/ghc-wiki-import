...
- `HsSyn` is parameterised over the types of the variables it contains.  The first three passes of the compiler work like this:
  ...
  - The **renamer** transforms this to `HsSyn` parameterised by {{{Name}}?.  To a first appoximation, a `Name` is a string plus a `Unique` (number) that uniquely identifies it.
