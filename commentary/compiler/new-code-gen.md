...
See also: [wiki:NewCodeGenOverview overview of the module structure in the new code generator].
 ...
 * '''Step 2''': Replace the existing Stg to Cmm code generator (a very complex and inflexible pass) with a new modular pipeline. The output of this pipeline is fed to the existing, un-modified code generators.  The design of the new pipeline is here: [wiki:Commentary/Compiler/NewCodeGenPipeline].
