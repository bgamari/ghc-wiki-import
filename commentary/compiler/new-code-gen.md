...
(May 2010: these workflow notes are written under the assumption that we have merged the current `ghc-cmm` branch with the HEAD, which I hope David will do shortly.)

We have the following repositories:

 * HEAD: the main GHC Darcs repo. Location: `http://darcs.haskell.org/ghc`

 * !HooplMaster: the master Hoopl Git repository.  Location: `tufts somewhere`
