...
The ticket for this request is [\#5462](https://gitlab.staging.haskell.org/ghc/ghc/issues/5462). For this discussion, we'll refer to this new feature as `GeneralizedDeriving`.
...
1. The typical generics class has one default signature, and perhaps a few more methods with a standard default. In this case, if there is no `MINIMAL` specified, it defaults to the empty set (as all methods have a default), so the class will be derivable.
...
1. Conversely, if GND is off, and `GeneralizedDeriving` is on, then we do the latter.
1. If both are on, but the class being derived does not have an empty `MINIMAL` set, we do GND.
1. If both are on, and the class being derived has an empty `MINIMAL` set, we fail, because it's ambiguous.
