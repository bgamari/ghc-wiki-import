...
On Posix, the timer signal is implemented by calling `setitimer()` to generate regular `SIGALRM` signals (the single threaded RTS uses SIGVTALRM).  This isn't ideal, since we'd like to allow the application t so use `SIGALRM` if it needs to.  Ideally we should use something better (see #850).
...
interrupts due to alarm signals. Most C libraries we deal with don't install signal handlers, so this is a not an issue. However, if the C library *does* install a signal handler, you'll 
need to be careful how it interprets the time signal from GHC.
