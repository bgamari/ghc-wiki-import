...
see [[GhcFile(rts/StgStdThunks.cmm)]].
...
the [wiki:Commentary/Rts/HaskellExecution/FunctionCalls#Genericapply generic apply functions] in !AutoApply.cmm.
 ...
 * The payload is the sequence of arguments already applied to
   ...
   [[GhcFile(rts/sm/Scav.c)]] for an example of traversing a PAP).
 ...
 * {{{IND_STATIC}}}: a static indirection, arises when we update a {{{THUNK_STATIC}}}.  A new {{{IND_STATIC}}}
   is placed on the mutable list when it is created (see {{{newCaf()}}} in [[GhcFile(rts/sm/Storage.c)]]).
