...
- The *layout* field describes the layout of the closure for the garbage collector.  The GC needs to know
  two things: how many words there are in the payload, and which of those words are pointers.  There are various ways
  in which a closure may be laid out:

  - *fixed layout*: the layout is fixed by the closure type, the layout field in the info table is ignored.  eg.
    MUT\_VAR closures have fixed layout.
  - *pointers first*: the payload consists of zero or more pointers followed by zero or more non-pointers.
  ...
  - *bitmap*: the layout is described by a bitmap (see [bitmaps](commentary/rts/heap-objects#bitmaps), below).  Stack frames generally have bitmap layout.
