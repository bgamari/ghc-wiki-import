...
== Files in `$(TOP)` ==
 ...
 '''`sync-all`'''::
  [wiki:Building/SyncAll This script] allows you to get or pull all the additional repositories that you need to build GHC.  The command-line interface is documented in the file itself.
 ...
 '''GNU autoconf machinery''':: `aclocal.m4`, `config.guess`, `config.sub`, `configure.ac`, `install-sh`, `config.mk.in`

 '''`ghc.spec.in`''':: the RPM spec file

 '''`ghc.mk`''', '''`Makefile`''':: The top-level {{{Makefile}}}: see [wiki:Building/Architecture GHC Build System Architecture]. GHC requires
