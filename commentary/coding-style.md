...
Import library modules from the base and haskell98 packages only. Use \#defines in HsVersions.h when the modules names differ between versions of GHC (eg. DATA\_IOREF in the example above). For code inside \#ifdef GHCI, don't need to worry about GHC versioning (because we are bootstrapped). 
