...
- TODO Type-indexed `Typeable` changes
  ...
  - `mkFunTy`, `mkAppTy`, `mkTyConApp`, and `` have been removed. If you use these then you p
  - `mkTyCon3` and `mkTyConApp` are no longer exported by `Data.Typeable` as it can be used to generate . It is instead exported by `Data.Reflection.Unsafe`.
