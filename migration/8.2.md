...
Before this change, GHC allowed multiple equations for instances of `T`. That is, this used to be allowed before 8.2:


```
instance C Int Char where
  type T Int Bool Char = Bool
  type T Int ()   Char = ()
  type T Int v    Char = v
```


This is now disallowed. If you want multiple cases like this, the solution is to use an auxiliary type family:


```
instance C Int Char where
  type T Int v Char = CIntChar v

type family CIntChar v where
  CIntChar Bool = Bool
  CIntChar ()   = ()
  CIntChar v    = v
```
