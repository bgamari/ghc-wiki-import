...
- Well-formedness of declarations involving families:
  ...
  - [\#1968](https://gitlab.staging.haskell.org/ghc/ghc/issues/1968) (GADT syntax in family instances; at least provide a proper error message and don't panic!)
  ...
  - Addition to user manual, see [
    http://www.haskell.org/pipermail/haskell-cafe/2008-March/040989.html](http://www.haskell.org/pipermail/haskell-cafe/2008-March/040989.html) and [http://www.haskell.org/ghc/docs/latest/html/users\_guide/data-type-extensions.html\#type-synonyms](http://www.haskell.org/ghc/docs/latest/html/users_guide/data-type-extensions.html#type-synonyms)
  - [\#2203](https://gitlab.staging.haskell.org/ghc/ghc/issues/2203) (TFs in class instance heads)
  - [\#2435](https://gitlab.staging.haskell.org/ghc/ghc/issues/2435) (Bug with qualified names in declarations)
  - [\#2436](https://gitlab.staging.haskell.org/ghc/ghc/issues/2436) (Bad warning on export)

- Solving of equalities (`TcTyFuns`):

  - Fix the problem reported [
    here](http://www.haskell.org/pipermail/haskell-cafe/2008-July/044911.html), and add a test case to the test suite.  The issue is whether to apply (Decomp) to an application of a saturated application of a type-function.
  - [\#2219](https://gitlab.staging.haskell.org/ghc/ghc/issues/2219), [\#2235](https://gitlab.staging.haskell.org/ghc/ghc/issues/2235), [\#1775](https://gitlab.staging.haskell.org/ghc/ghc/issues/1775) & test `GADT1` (bogus occurs check failure - in both bugs, the loop is through a TF)
  - [\#2202](https://gitlab.staging.haskell.org/ghc/ghc/issues/2202) (Uses `a ~ MeshVertex a b` in `normaliseWantedDicts` w/o the occurs check kicking in; also occurs in 6.8.2 and the program doesn't mention TFs, so need to merge):
  ...
  - [\#2146](https://gitlab.staging.haskell.org/ghc/ghc/issues/2146) (infelicity in decomposition for higher-order TFs)
  - [\#2448](https://gitlab.staging.haskell.org/ghc/ghc/issues/2448) (givens not properly used in superclass entailment check)
  - [\#2102](https://gitlab.staging.haskell.org/ghc/ghc/issues/2102) (superclass equalities)
...
- GADT:
  ...
  - [\#1999](https://gitlab.staging.haskell.org/ghc/ghc/issues/1999) (barfs with corelint) & [\#2432](https://gitlab.staging.haskell.org/ghc/ghc/issues/2432) (fails in same function, but also w/o corelint)
  - [\#2212](https://gitlab.staging.haskell.org/ghc/ghc/issues/2212) (Assertion failure in `writeMetaTyVar` with -DDEBUG on gadt/equal; see also below, and check [\#2231](https://gitlab.staging.haskell.org/ghc/ghc/issues/2231) and [\#2366](https://gitlab.staging.haskell.org/ghc/ghc/issues/2366) which are probably duplicates).  Simon found the exact cause: it's in TcSimplify where we abandon an implication constraint despite having solved some equalities.
  - [\#2151](https://gitlab.staging.haskell.org/ghc/ghc/issues/2151) (nested GADT constructors in patterns)
  - [\#2040](https://gitlab.staging.haskell.org/ghc/ghc/issues/2040) (incomplete deduction of evidence for class contexts in GADT constructors)

- Misc:

  - Test `Simple17` & `GADT12` (corelint errors)
  - [\#2291](https://gitlab.staging.haskell.org/ghc/ghc/issues/2291) (panic mixing RULES and type families; rule simplification stumbles over a coercion)
  - [\#714](https://gitlab.staging.haskell.org/ghc/ghc/issues/714) (feature request: fundeps treated inconsistently in superclasses and type sigs)
  - [\#1897](https://gitlab.staging.haskell.org/ghc/ghc/issues/1897): If you infer a type for a function, then should check the function against that sigature, to check that if the user gave that signature, then typechecking would again succeed.  See this thread [
    http://www.haskell.org/pipermail/haskell-cafe/2008-April/041385.html](http://www.haskell.org/pipermail/haskell-cafe/2008-April/041385.html).  [\#2418](https://gitlab.staging.haskell.org/ghc/ghc/issues/2418) suggests that for higher-kinded TFs, we could use decomposition more aggressively.
...
- Implementing FDs by TFs:
  ...
  - ghci command to print normalised type and add [
...
1. Single phase algorithm; cf `single_phase_algorithm.tex`.
  ...
  - `substEqInDict` needs to be symmetric (i.e., also apply right-to-left rules); try to re-use existing infrastructure.  It would be neater, easier to understand, and more efficient to have one loop that goes for a fixed point of simultaneously rewriting with given\_eqs, wanted\_eqs, and type instances.
...
- Deriving `Typeable` for data families ([\#1769](https://gitlab.staging.haskell.org/ghc/ghc/issues/1769))
