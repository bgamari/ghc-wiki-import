 ...
 * Solving of equalities (`TcTyFuns`):
  * Fix the problem reported [http://www.haskell.org/pipermail/haskell-cafe/2008-July/044911.html here], and add a test case to the test suite.  The issue is whether to apply (Decomp) to an application of a saturated application of a type-function.
  * #2219, #2235, #1775 & test `GADT1` (bogus occurs check failure - in both bugs, the loop is through a TF)
  * #2202 (Uses `a ~ MeshVertex a b` in `normaliseWantedDicts` w/o the occurs check kicking in; also occurs in 6.8.2 and the program doesn't mention TFs, so need to merge):
   ...
   * #2448 (givens not properly used in superclass entailment check)
 ...
 * GADT:
  * #1999 (barfs with corelint) & #2432 (fails in same function, but also w/o corelint)
  * #2212 (Assertion failure in `writeMetaTyVar` with -DDEBUG on gadt/equal; see also below, and check #2231 and #2366 which are probably duplicates).  Simon found the exact cause: it's in TcSimplify where we abandon an implication constraint despite having solved some equalities.
...
'''Debugging of type families:'''
 0. Open points in the new `TcTyFuns` code:
    * extract from `TODO`s in `TcTyFuns`
 0. Issues in `TcSimplify`:
 ...
 0. RankN: When can foralls appear in equalities?  What constraints does that place on GADTs?  Also, the code in `TcTyFuns` doesn't really deal with rank-n types properly, esp `decompRule`.  Also test `Simple14` & `GADT10`.
