...
- Add the program of [\#1900](https://gitlab.staging.haskell.org/ghc/ghc/issues/1900) & [\#1999](https://gitlab.staging.haskell.org/ghc/ghc/issues/1999) and [\#1834](https://gitlab.staging.haskell.org/ghc/ghc/issues/1834) (but needs to use ghci)
...
- Well-formedness of declarations involving families:
  ...
  - [\#2157](https://gitlab.staging.haskell.org/ghc/ghc/issues/2157) (solution: lhs of type instances may not contain partially applied vanilla type synonyms)
...
- Solving of equalities (`TcTyFuns`):
  ...
  - [\#2219](https://gitlab.staging.haskell.org/ghc/ghc/issues/2219), [\#2235](https://gitlab.staging.haskell.org/ghc/ghc/issues/2235) & [\#1775](https://gitlab.staging.haskell.org/ghc/ghc/issues/1775) (bogus occurs check failure - in both bugs, the loop is through a TF)
...
- GADT:
  ...
  - [\#2212](https://gitlab.staging.haskell.org/ghc/ghc/issues/2212) (Assertion failure in `writeMetaTyVar` with -DDEBUG on gadt/equal; see also below)
...
1. RankN: When can foralls appear in equalities?  What constraints does that place on GADTs?  Also, the code in `TcTyFuns` doesn't really deal with rank-n types properly, esp `decompRule`.  Also test `Simple14`.
