...
https://github.com/ghc-ios/ghc-ios-scripts](https://github.com/ghc-ios/ghc-ios-scripts) (The problem is that 1. GHC tries to run a C pre-processor over Haskell code, and 2. Xcode versions \>= 5 use clang instead of gcc, and 3. clang doesn't like Haskell code.). To use it, compile clang-xcode5-wrapper.hs, add it to your path, then edit /usr/local/lib/ghc-7.6.3/settings and change "C compiler command"'s value to "clang-xcode5-wrapper".
...
- Programs outputting substantial text to the console can cause hangs in the RTS. (fixed by lukexi in ticket [\#8307](https://gitlab.staging.haskell.org/ghc/ghc/issues/8307)).
