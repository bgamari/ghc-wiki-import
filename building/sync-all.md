...
```trac
'''Note''': This page is here mostly for historical reasons. A GHC tree used to consist of multiple independent  repositories. The `sync-all` script let you operate over them all at once. It is still present in the root directory of the tree, but since all [wiki:Repositories repositories] have been turned into git submodules, you normally don't need it anymore. See [WorkingConventions/Git git working conventions] and [WorkingConventions/Git/Submodules git submodules] for the new submodule workflow.
