...
The [wiki:Building/DarcsAll darcs-all script] adds the `--partial` flag by default.
...
However, '''you cannot use `darcs get` to get a full GHC repository''', for two reasons:
  ...
  * Darcs prior to version 2.3 has a bug concerning case-sensitivity on Windows, and ([http://www.haskell.org/pipermail/glasgow-haskell-users/2007-November/013373.html apparently]) MacOS X, which makes Darcs crash if you do `darcs get` on the full GHC repository.  You get this message
{{{
...
The [wiki:Building/DarcsAll darcs-all] script makes it easy to pull new patches.
