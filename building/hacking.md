...
Another way to build GHC is to [wiki:TestingPatches run the validate script], which does a build of GHC from scratch (it does `make distclean` first), using standard settings, and then runs the testsuite.  If validate passes without any errors, then it is ok to submit the patches from your tree.
