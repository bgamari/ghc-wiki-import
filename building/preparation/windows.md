...
- Create priviledged `cyg_server` user (required in most current Windows versions)
  ...
  # the SSH service should start up automatically when the Windows VM is rebooted. You can manually restart the service by running `net stop ssh` + `net start ssh`
  net start ssh
