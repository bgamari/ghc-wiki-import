...
Download and run the [http://sourceforge.net/projects/msys2/files/latest/download msys2 installer (64-bit)] (a [http://sourceforge.net/projects/msys2/files/Base/i686/ 32-bit version] is also available). Open a mingw64 shell.
...
,,(Installing winpthreads is a workaround for the in-place gcc not being smart enough to find its own pthread DLLs.),,
...
{{{
curl http://www.haskell.org/ghc/dist/7.8.3/ghc-7.8.3-$(uname -m | sed -e s/686/386/)-unknown-mingw32.tar.xz | tar -xJ -C /tmp &&
mkdir -p /usr/local &&
...
(Note: the 'alex' build appears to be broken on the 32-bit platform, see #9712.)
