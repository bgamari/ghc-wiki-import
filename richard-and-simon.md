...
- Implement [\#13650](https://gitlab.staging.haskell.org/ghc/ghc/issues/13650) (subsumes `mkCastTy` stuff below)
...
- Simplify `mkCastTy`, keeping the `isReflexiveCo` check. Document the reflexivity invariant. (Do we need to differentiate between `tcView`-reflexivity and `coreView`-reflexivity?) ([\#13650](https://gitlab.staging.haskell.org/ghc/ghc/issues/13650))
