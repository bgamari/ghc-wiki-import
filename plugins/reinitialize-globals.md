...
After a few, unfortunately public, iterations, I'm pushing Solution 1.


- Solution 1 is a lightweight solution, re-uses an existing mechanism, has a small footprint in the GHC source code, is totally transparent to the plugin author, and robustly handles corner cases. 

- Solution 2 only simplifies code, but it makes a user choose between (safely) using a plugin that involves `FastString.string_table` and using statically-linked GHC.


Solution 1 relies on the `rts/Globals.c`, which would be excised anyway if Solution 2 is ever viable…
...
- [\#3658](https://gitlab.staging.haskell.org/ghc/ghc/issues/3658) — this is for GHCi, but it might carry over for ghc; that's an open question

- [\#8039](https://gitlab.staging.haskell.org/ghc/ghc/issues/8039) — might be blocking 3658
