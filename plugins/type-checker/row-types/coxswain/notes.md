...
1. If both roots are variables, then we "articulate": we create a new implicit variable and extend the substitution with correspondings mappings for the two root variables (`x .& B ~ y .& A` becomes `x := alpha .& A` and `y := alpha .& B` for a fresh `alpha`).
...
Note that the orderless semantics of set forms means that `[LacksE]` generalizes to set inclusion of the extension elements, as demonstrated in the `{u,v}` and `{u,v,w}` example in the previous paragraph.
