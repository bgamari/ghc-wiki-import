...
one of `p` and `q` with fresh touchable vars. For `WiderThan`, it's to
replace the occurrence of `Row0` in the narrower row (`p`) with a fresh
touchable var. Note however that this approach does not provide
...
1. Immediately upon receiving Givens, thaw untouchable `Row` vars, i.e. replace them by touchables. This enables certain Given-Given interactions, specifically "unification" via the GHC type equality solver rules EQSAME, EQDIFF, etc. especially involving flatten skolems.


This allows GHC's type equality solver to "learn" the equivalences between (untouchable) `Row` vars for me.


1. Importantly, when renaming the untouchables, I also create some fake Given constraints `Ren p_sk p_tau` where `p_tau` is the temporary touchable replacement of the untouchable `p_sk`. This is a robust way for me to later recover the `Row` var equalities that GHC's solver learned for me.

1. When finished simplifying givens (i.e. if I would be returning `TcPluginOk _ []`), replace any remaining touchable variables (We created all touchable variables, right? ... Do touchables otherwise arise in Givens?) by signature skolems if possible or fresh skolems if not. Such fresh skolems correspond to row restrictions (i.e. removing one or more columns) of one or more signature skolems.


The motivation here is that having touchables in the Givens while simplifying Wanteds is A Bad Thing (e.g. loss of "confluence"). Also note that you cannot solve/create Givens when simplifying Wanteds, so this step has to happen when simplifying Givens.
...
- When do touchables "naturally" arise in Given constraints? (My plugin relies on the fact that GHC's type equality solver seems to expect touchables in Given constraints, but I've never seen them there except when I create them. So I'm worried they're not supposed to be there.)

- When do untouchables arise in Wanted constraints without having first arisen in Given constraints? I've seen this happen for ambiguity checks, I think, but that's it. Is that the only time it happens?
