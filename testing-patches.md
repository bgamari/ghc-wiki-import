...
When you [wiki:WorkingConventions/FixingBugs contribute a patch] to GHC, it has to be validated before we push it to the master branch.

First, [wiki:Building/RunningTests/Adding add a new test], and [wiki:Building/RunningTests/Running run it].
...
=== Phabricator ===
Any patch submitted to [wiki:Phabricator] is automatically validated by [wiki:Phabricator/Harbormaster Harbormaster].
...
* Fork the GHC repository on [https://github.com/ghc/ghc Github].
* Sign up for [https://travis-ci.org/ Travis CI].
...
* If you make a fork of any of the submodule repositories (i.e. [https://github.com/ghc/packages-filepath ghc/packages-filepath]), Travis will use it instead of the original. This is feature that Phabricator doesn't provide at the moment.
* Settings are in the file `.travis.yml`, though you shouldn't have to change anything. See the [wiki:Travis Travis wiki page] for details.
...
In order to save time while debugging problems revealed by validate, the validate script understands several flags. These flags, and others understood by `validate` are documented in the `validate` script itself.
  ...
  * '''`--fast`''':  validate skips the build-a-distribution step, and does not build the 'extra' packages (i.e. does not set `BUILD_EXTRA_PKGS=YES`, see [wiki:Building/Using#Buildconfiguration]). 
