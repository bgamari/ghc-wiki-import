...
('''Qnikst''': forth rule (about unboxed tuples) was not added to spec (source:docs/core-spec/CoreLint.ott) yet)  '''SPJ''' What spec?  Give a link.
...
'''SPJ''' Actually it should be ''unboxed'' not ''unlifted''.  It's wrong to cast between `Array# Int` and `Int#` because the former is a pointer and the latter is not.
  ...
  4. If types are unboxed tuples then tuple `(# A_1,..,A_n #)` can be coerced to `(# B_1,..,B_m #)` if `n=m` and for each pair `A_i, B_i` rules 1-4 holds.
