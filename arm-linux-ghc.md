...
##
New Update on [\#1346](https://gitlab.staging.haskell.org/ghc/ghc/issues/1346)



[\#1346](https://gitlab.staging.haskell.org/ghc/ghc/issues/1346), the bug tracking having bootstrapping working again, has had its milestone changed from 6.10.1 to 6.12, as the build system overhaul has grown into too large of a project and 6.10 needs to be released before it will be completed.
...
I had to install 6.6.1 on my system to compile 6.6.1, first off. Then I apparent need an older gcc (have 4.3.0). Rather than dive deeper into that (it would require an old gcc on the scratchbox dev environment too, too much pain when I have an alternative), I'm just going to try to build an unregisterised 6.8.2 with the patch posted in [\#1346](https://gitlab.staging.haskell.org/ghc/ghc/issues/1346). The results for that build attempt follow, after the info for the failed 6.6.1 one.
...
Using the patch posted at [\#1346](https://gitlab.staging.haskell.org/ghc/ghc/issues/1346), I'm trying to build 6.8.2.
...
On discussion with Igloo (Ian Lynagh) on \#haskell, it turns out that hc-file bootstrapping has been broken in 6.8 since the beginning, meaning that only platforms with a working GHC prior to 6.8 can build a 6.8 or later. This undercuts the porting effort until [\#1346](https://gitlab.staging.haskell.org/ghc/ghc/issues/1346) is fixed. I may try the "quick hack" diff attached to see if it works out. The comment from the user who posted it seems to imply that it's incomplete, though.
