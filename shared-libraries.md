...
When building an executable against shared libraries, libtool links the executable with no paths embedded (for installation), and stores this in `.libs`.  It also creates a script for running the program in-place.  The script relinks the executable against uninstalled shared libraries (using `-rpath` on Linux) on demand, caches the resulting executable in `.libs`.
