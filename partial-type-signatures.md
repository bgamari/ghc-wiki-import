...
'''UPDATE''' See the section [#implementation implementation] and its
...
[#constraint-wildcards unify with something]. Extra-constraints
...
1. TODO A partial type signature for a local definition is not generalised
   ...
   is not ''that'' common (see [http://research.microsoft.com/apps/pubs/default.aspx?id=162516 Section 4.3.1] of
   ''[[span(!OutsideIn(X), style=font-variant: small-caps)]]: Modular type inference with local assumptions'').
...
2. TODO There is a bug in the handling of pattern bindings with partial type
   ...
   This doesn't seem to be difficult to solve, but requires some debugging.
...
6. There are two actual TODOs in the code:
   [[br]][[br]]
    ...
    2. TODO In the local function `makeWanted` in `tcPolyInfer`, in `TcBinds.lhs`,
       we create an `Implic`ation without skolem information, i.e. `UnkSkol`.

7. TODO The user manual should be updated. We'll do this when the time
   comes.

8. TODO We extended Haddock to support partial type signatures, but we
