...
- Currently I disable the Core Lint check for unsaturated applications of unlifted types; it's not clear whether this is actually a safe thing to do; tracked as [\#11736](https://gitlab.staging.haskell.org/ghc/ghc/issues/11736)
- Look at the testsuite failures:
  ...
  - others are Core lint violations:
    ...
    - `T11120`, due to the overly-restrictive kind of `(->)`, being tracked as [\#11714](https://gitlab.staging.haskell.org/ghc/ghc/issues/11714)
  - others apparently incorrect output:

    - `TypeOf`, due to the fact that `Constraint` and `*` are indistinguishable in Core, being tracked as [\#11715](https://gitlab.staging.haskell.org/ghc/ghc/issues/11715)
