...
This page gathers the notes about implementing new primitive logical operations and thus resolving ticket [\#6135](https://gitlab.staging.haskell.org/ghc/ghc/issues/6135).
...
- as a small side-task, four new logical bitwise primops have been implemented: `andI#`, `orI#`, `xorI#` and `negI#` ([\#7689](https://gitlab.staging.haskell.org/ghc/ghc/issues/7689)). These operate on values of type `Int#`. Earlier we had only bitwise logical primops operating on values of type `Word#`.
...
`R2` contains the first argument `x`, `R3` contains `y`, `R4` contains `width` and `R5` contains `height`. We can verify that by looking at the body of Main.f\_info:
...
```wiki
  ...
  shrq   $63, %rax    # shift %rax 63 bits to the right. If both x and y were
                      ...
                      # is now 1
  cmpq   %rdi, %r14   # subtract width (in %rdi) from x (in %r14), discard the
                      # result and set the flags. If width was greater than x
                      # then SF is set to 1 and OF is set to 0. This is tricky
                      # because combination of SF and OF is needed to decide
                      # which argument is actually equal 
  setge  %cl          # if OF=SF it means that x was greater or equal to width
                      # and we note that fact by setting LSB in %cl to 1
