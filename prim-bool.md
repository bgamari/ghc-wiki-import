...
Thanks to these wrappers the change is almost backwards compatible. **The only thing primop users need to change in their existing code to make it work again is adding import of GHC.PrimWrappers module.** The only exception are primops that operate on TVars, MVars and MutableArrays
