...
- Applications of indexed types need to supply all indexes; i.e., partial application to indexes is not admitted.  (Arguments beyond the indexes can be partially supplied as usual.)  **We probably want the saturation constraint only for type functions, not for data/newtype families.**
