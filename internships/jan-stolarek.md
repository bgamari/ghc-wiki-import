...
- [\#605](https://gitlab.staging.haskell.org/ghc/ghc/issues/605) - Optimisation: strict enumerations
...
- [\#2450](https://gitlab.staging.haskell.org/ghc/ghc/issues/2450) - Data.Complex.magnitude squares using <sup>(2 :: Int), which is slow
  </sup>
- [\#2731](https://gitlab.staging.haskell.org/ghc/ghc/issues/2731) - Avoid unnecessary evaluation when unpacking constructors
- [\#2823](https://gitlab.staging.haskell.org/ghc/ghc/issues/2823) - Another arity expansion bug
- [\#4470](https://gitlab.staging.haskell.org/ghc/ghc/issues/4470) - Loop optimization: identical counters
- [\#4937](https://gitlab.staging.haskell.org/ghc/ghc/issues/4937) - Remove indirections caused by sum types, such as Maybe
...
- [\#7574](https://gitlab.staging.haskell.org/ghc/ghc/issues/7574) - Register allocator chokes on certain branches with literals (bug can be triggered with `./inplace/bin/ghc-stage2 -c -no-hs-main -fasm -O2 ./testsuite/tests/llvm/should_compile/T7571.cmm`)
- [\#8048](https://gitlab.staging.haskell.org/ghc/ghc/issues/8048) - Register spilling produces ineffecient/highly contending code
